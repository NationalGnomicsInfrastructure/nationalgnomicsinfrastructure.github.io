...
- **Platform section** — `src/pages/platform/` contains an interactive multi-step workflow (access-code verification → species identification → project retrieval). State is kept in `localStorage` under the keys: `team`, `leader`, `species_done`, `spirit_animal`, `accession`, `code_idx`, `mode`.
